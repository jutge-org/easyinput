...
def test_1():
    ...
    input_gen = jutge.read_while(file=source)
    assert all(next(input_gen) == val for val in expected)


def test_2():
    source = open('./test/text/source1.txt', 'r')
    assert not tuple(jutge.read_while(int, file=source))
...
def test_3():
    ...
    input_gen = jutge.read_while(int, file=source)
    assert all(next(input_gen) == val for val in expected)
...
def test_4():
    ...
    input_gen = jutge.read_while(float, file=source)
    assert all(next(input_gen) == val for val in expected)
...
def test_5():
    ...
    input_gen = jutge.read_while(str, int, file=source)
...
def test_6():
    ...
    input_gen = jutge.read_while(int, file=source, amount=3)
...
def test_7():
    ...
    input_gen = jutge.read_while(int, file=source, amount=5)
    assert all(tuple(next(input_gen)) == tuple(val) for val in expected)
